...
*   **Arquitectura de Backend:** Se implementará un patrón de **Capa de Servicios (Service Layer)**. Toda la lógica de negocio principal (crear reservas, buscar músicos, etc.) se centralizará en clases de servicio dedicadas en `app/Services`. Esto desacopla la lógica de los controladores y componentes de Livewire, permitiendo su reutilización y facilitando las pruebas. Se usarán **Data Transfer Objects (DTOs)** con `spatie/laravel-data` para estandarizar el flujo de datos entre capas.
*   **API:** **API RESTful (API-First)**. El objetivo es exponer toda la lógica de negocio de la capa de servicios a través de una API RESTful completa. Este enfoque "API-First" garantiza que todas las funcionalidades estén disponibles programáticamente, lo que facilitará el desarrollo de futuras aplicaciones móviles (iOS/Android) y la integración con servicios de terceros. La documentación seguirá el estándar **OpenAPI (Swagger)**.
