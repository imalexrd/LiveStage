    ...
    <div class="p-6 bg-white border-b border-gray-200">
        ...
        <div class="mb-6">
            ...
            @if ($profile->banner_image_path)
                <div class="mt-4">
                    <img src="{{ route('storage.serve', ['path' => $profile->banner_image_path]) }}" alt="Banner Image" class="w-full h-auto rounded-lg shadow-md">
                </div>
            @endif
        </div>
        ...
        <div class="mb-6">
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                @foreach($media->where('type', 'image') as $image)
                    <div class="relative">
                        <img src="{{ route('storage.serve', ['path' => $image->file_path]) }}" alt="Gallery Image" class="w-full h-auto rounded-lg shadow-md">
            ...
            @foreach($media->where('type', 'video') as $video)
                <div class="mt-4 relative">
                    <video controls class="w-full h-auto rounded-lg shadow-md">
                        <source src="{{ route('storage.serve', ['path' => $video->file_path]) }}" type="video/mp4">
            ...
            @foreach($media->where('type', 'audio') as $audio)
                <div class="mt-4 relative">
                    <audio controls class="w-full">
                        <source src="{{ route('storage.serve', ['path' => $audio->file_path]) }}" type="audio/mpeg">
